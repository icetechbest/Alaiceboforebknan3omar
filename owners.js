    ...
    async monitor(sock, m, db) {
        ...
        if (text.includes('chat.whatsapp.com/') || text.includes('@everyone') || text.includes('@here')) {
            ...
            if (!isAdmin) {
                ...
                // 📝 سجل الطرد بقى بيتبعت في نفس الجروب مع منشن للشخص المطرود، بدل
                // ما يتبعت خاص لرقم الأونر (كان بيساهم في تبنيد البوت لو حصل كتير).
                await sock.sendMessage(id, { text: `🚨 *سجل سونج*\nتم طرد @${sender.split('@')[0]} لنشره رابط/تاغ جماعي في هذا الجروب.`, mentions: [sender] });
                return;
            }
        }
        ...
        if (recent.length > 5) {
            ...
            if (!isAdmin) {
                await sock.sendMessage(id, { delete: m.key });
                await sock.groupParticipantsUpdate(id, [sender], "remove");
                await sock.sendMessage(id, { text: `⚡ *سجل سونج*\nتم طرد @${sender.split('@')[0]} بسبب السبام في هذا الجروب.`, mentions: [sender] });
            }
        }
    }
};
